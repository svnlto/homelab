...
- **Kubernetes**: Talos K8s cluster on Proxmox with Flux Operator GitOps (arr-stack, Jellyfin, etc.)
...
│   └── flux-operator/       # Flux Operator + FluxInstance deployment
├── prod/
...
│   ├── compute/flux-operator/ # Flux Operator on k8s-shared
│   ├── storage/
...
### Kubernetes (Flux Operator GitOps)
...
kubernetes/
├── flux/
│   ├── clusters/shared/     # FluxInstance sync entry point (infra.yaml, apps.yaml)
│   ├── infra/               # Namespaces, HelmRepositories, ConfigMaps (from values files)
│   └── apps/                # ResourceSets for HelmReleases, image automation
├── apps/<app>/overlays/<cluster>/values.yaml  # Per-cluster Helm values
└── argocd-apps/             # ArgoCD Application manifests (legacy, being removed)
```

- Single Talos cluster (`k8s-shared`, VLAN 30) managed via Terragrunt
- Flux Operator deploys FluxInstance which syncs from `kubernetes/flux/clusters/shared/`
- **ResourceSets** template HelmReleases — one for local charts, one for external charts
- Values delivered via **configMapGenerator** → ConfigMaps consumed by `valuesFrom`
- External apps (OpenObserve, vcluster, CNPG, OTel) use upstream Helm charts
- Image automation (dumper, osxphotos-export) via ImageRepository + ImagePolicy + setter markers
- Chart READMEs are auto-generated by `helm-docs` — don't edit manually
- Storage via democratic-csi (NFS + iSCSI from TrueNAS) — Terraform-managed, not Flux-managed
- Flux web UI at `flux.shared.h.svenlito.com`
