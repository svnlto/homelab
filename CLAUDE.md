...
- ArgoCD watches `kubernetes/argocd-apps/` for app-of-apps pattern
- Apps use **Helm charts** in `charts/` with per-cluster values in `kubernetes/apps/`
- ArgoCD uses **multi-source pattern**: one source for the chart, one `ref: values` source for the values file
- External apps (e.g. SigNoz) use upstream Helm charts, not local ones
