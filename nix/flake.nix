  ...
  outputs =
    ...
    in
    {
      packages.x86_64-linux.osxphotos-export-image =
        let
          osxphotos-linux = pkgs-x86.python312Packages.osxphotos.overridePythonAttrs (old: {
            dependencies = builtins.filter (dep: (dep.pname or "") != "utitools") (old.dependencies or [ ]) ++ [
      ...
      nixosConfigurations = {
        ...
        rpi-devbox = nixpkgs.lib.nixosSystem {
          system = "aarch64-linux";
          modules = [
            "${nixpkgs}/nixos/modules/installer/sd-card/sd-image-aarch64.nix"
            ./rpi-devbox/configuration.nix
            {
              # Faster builds (disable compression)
              sdImage.compressImage = false;
            }
